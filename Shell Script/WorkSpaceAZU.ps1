#region 0 - Descrição, Parâmetos, Constantes, Funções ------------------------------------------------------------------------------------------
	#region Descrição
		...
		#	Exemplos:
		#				. .\WorkSpace.ps1 -Oferta AS-CS_POWERBI_APL -CodApl YGLQ -Registro sala -ChaveRegistro chave -Premium CL_002
		#				. .\WorkSpace.ps1 -Oferta AS-CS_POWERBI_APL -CodApl N1JV -Registro sala -ChaveRegistro chave -Premium CL_001 -EmailEnvia 0 -ConAzureManual 1
		#				. .\WorkSpace.ps1 -Oferta AS-CS_POWERBI_GRUPO_SEGURANCA -SufixoGrupoSegGeral TESTE1 -Registro SXXXXX -ChaveRegistro chave -CodApl PBIP -EmailEnvia 0
		#
...
#		[Parameter( ParameterSetName="GrupoSeg", Mandatory=$true  )]								#
	...
	$idCapacidade			= 'D677CDA4-CB9AA7-F47B61168B01'				# Id da Capacidade da POC
	$pbiAdminId				= '2832d2e0-a2ea-c941e5cc6bef'				# (Get-AzureADGroup -SearchString  GN__POWER-BI_ADMINS).ObjectId
		# '1d45588d-0bae--f91b7c1fe3d5'								# GN__POWERBI_ADMINS   ( Sem traco do Power-bi só SaPowerbi )
		# '2832d2e0-a2ea--c941e5cc6bef'	
	$pbiDesenvId 			= '6f54a711-3aa4-4561-9fa0-9ba14243c31f'				# GN__POWER-BI_DEV_TIC
		#'96e2ccaf-9fee-45a9-9940-d61f6a666196'									# PBI_DESENOLVEDOR_TIC
	$pbiGovernancaId		= 'c37755b6-6d5b-410d-9644-c0b409449127'				# GN__POWER-BI_GOV
...
mostra $et.10
<# sqlplus /nolog
# Desenv: ( DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=npaa5968.teste.biz)(PORT=1521)) (CONNECT_DATA=(SID=oradsv11)) )
conn sapowerbi/"senha"@'( DESCRIP...'
# Prod:   ( DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=npaa5787.teste.biz)(PORT=1521)) (CONNECT_DATA=(SERVICE_NAME=oraprd11.teste.COM.BR)(INSTANCE_NAME=rcprd11a)) )
conn sapowerbi/"senha"@'( DESCRIP...'
#conn cmxl/"senha"@'(DESCRIPTION = (ADDRESS_LIST = (ADDRESS = (PROTOCOL = TCP)(HOST = npaa2654.teste.biz)(PORT = 1521)))(CONNECT_DATA = (SERVICE_NAME = vcxlh)(SERVER = DEDICATED)))'
SET PAGESIZE 999
SET LINESIZE 140
...
if ( $ConsultaCatOracle ) {
	...
	$datasource = "(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=npaa5787.teste.biz)(PORT=1521))(CONNECT_DATA=(SERVICE_NAME=oraprd11.teste.COM.BR)(INSTANCE_NAME=rcprd11a)))"
...
if ( $ConsultaCatBarramento ) {
	...
	$URI = "https://bs.teste.com.br/CATAPL/Services/ProxyServices/Aplicacao_1"
	$xmlRec = @"
	<soapenv:Envelope xmlns:soapenv='http://schemas.xmlsoap.org/soap/envelope/' xmlns:ns='http://services.teste.biz/ti/aplicacao/1'>
		<soapenv:Header/>
		<soapenv:Body>
...
try {
	$MeuCred1 = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList "$usernamePowerBI1@ps.com.br", ( Get-Content $FilePass1 | ConvertTo-SecureString )
} catch {
	Mostra ( $Error[0] | out-string ) 2
	saierro 128
}
...
if ( ($Oferta -eq "AS-CS_POWERBI_APL") -or ($Oferta -eq "AS-CS_POWERBI_GRUPO_SEGURANCA") -or ($Oferta -eq "AS-CS_POWERBI_WS_DADOS") ) {
	...
	if ( $GrupoSegPadraoCria -or ( $Oferta -eq 'AS-CS_POWERBI_GRUPO_SEGURANCA' ) ) {
		...
		Add-AzureADGroupMember -ObjectId ( Get-AzureADGroup -Filter "DisplayName eq 'GN_PBI_AARE_VISUALIZADOR'" ).ObjectId -RefObjectId (Get-AzureADUser -ObjectId ")
		Add-AzureADGroupMember -ObjectId ( Get-AzureADGroup -Filter "DisplayName eq 'GN_PBI_AARE_VISUALIZADOR'" ).ObjectId -RefObjectId (Get-AzureADUser -ObjectId "r").ObjectId

		Add-AzureADGroupOwner -ObjectId ( Get-AzureADGroup -Filter "DisplayName eq 'GN_PBI_AARE_VISUALIZADOR'" ).ObjectId -RefObjectId (Get-AzureADUser -ObjectId "s.com.br").ObjectId
	...
	if ( $WorkspaceCria ) {
		try {
			...
			mostra "acesso Admin concedido na WorkSpace para GN__POWER-BI_ADMINS"
		} catch {
	...
	if ( $GrupoSegPadraoCria ) {
		...
		try {
			...
			mostra "acesso Member concedido na WorkSpace para GN_BS_POWER-BI_GOV"
		} catch {
			Mostra ( $Error[0] | out-string ) 2
			saierro 131
		}
		try {
			...
			mostra "acesso Contributor concedido na WorkSpace para GN__POWER-BI_DEV_TIC"
		} catch {
			Mostra ( $Error[0] | out-string ) 2
			saierro 132
		}
	...
	if ( $Oferta -eq 'AS-CS_POWERBI_WS_DADOS' ) {
		try {
			Add-PowerBIWorkspaceUser -AccessRight Member -Id $IdWsAtiva -PrincipalType Group -Identifier $pbiGovernancaId -ErrorAction Stop
			mostra "acesso Member concedido na WorkSpace para GN_PAS_POWER-BI_GOV"
		} catch {
			Mostra ( $Error[0] | out-string ) 2
			saierro 131
		}
		try {
			...
			mostra "acesso Member concedido na WorkSpace para GN_PS_POWER-BI_DEV_TIC"
		} catch {
			Mostra ( $Error[0] | out-string ) 2
			saierro 132
		}
	...
	Add-PowerBIWorkspaceUser  -Scope Organization -AccessRight Member -ID $ws.id -UserPrincipalName sizo.dn@.com.br
	...
	Add-PowerBIWorkspaceUser -AccessRight Member -Id $ws.id -PrincipalType User -Identifier cassius.serra@s.com.br
	#>
	function dbg {
		if ( $EquipeTeamsCria ) {
			mostra "Equipe Teams:"
...
#region 99 - Fim -------------------------------------------------------------------------------------------------------------------------------
<# Diversos:
$NomeGrupoMs365 = "TESTE Grupo criado separadamente"
$oGrupoMS365 = Get-AzureADGroup -SearchString $NomeGrupoMs365
...
$authheader = Get-PowerBIAccessToken
  $grpid = "4-34e9220680f6"
  #"39e07cba-85e3-9684e9a69b73"
  #"07187aa1-f87e-1724d7f50c66"		#"70626bb6-e6e8-4cb60445a7955"
